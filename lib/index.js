...
var DEFAULT_CLIENT_FIELD_OPTIONS = [
  {name: 'Other', value: 'client_other'}
]

var DEFAULT_PROJECT_FIELD_OPTIONS = [
  {name: 'Other', value: 'project_other'}
]

module.exports = function(done) {
  return getHarvestProjectsThenRelatedClients().spread(syncToZendesk);
}

function getHarvestProjectsThenRelatedClients() {
  var projects;
  ...
  return rp(requestParams).then(getClients);
}
...
function getClients(harvestProjects) {
  ...
  .then(function (harvestClients) {
    return [harvestClients, harvestProjects];
  })
}
